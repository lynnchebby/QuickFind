<template>
  <div class="min-h-screen bg-white text-gray-800 dark:text-neutral-200 overflow-hidden relative">
    <div class="absolute top-0 end-0 -translate-y-1/4 translate-x-1/4 z-0 opacity-30 animate-float-1 hidden md:block">
      <svg class="w-64 h-auto text-blue-200 dark:text-blue-800" fill="none" viewBox="0 0 200 200"
    ...
    <div
      class="absolute bottom-0 start-0 translate-y-1/4 -translate-x-1/4 z-0 opacity-30 animate-float-2 hidden md:block">
      <svg class="w-72 h-auto text-purple-200 dark:text-purple-800" fill="none" viewBox="0 0 250 150"
        xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="absolute top-1/4 left-1/2 -translate-x-1/2 z-0 opacity-20 animate-float-3 hidden md:block">
      <svg class="w-48 h-auto text-green-200 dark:text-green-800" fill="none" viewBox="0 0 150 150"
        xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="max-w-[85rem] px-4 py-6 sm:px-6 lg:px-8 mx-auto relative z-10">
      ...
      <div class="mx-auto max-w-3xl mb-8 lg:mb-12 text-center pt-4 md:pt-6">
        <h1
          class="text-6xl sm:text-8xl font-extrabold text-blue-600 dark:text-blue-400 drop-shadow-lg animate-fade-in-down">
          QuickFind
        </h1>
        <h2 v-if="user && user.name"
          class="mt-4 text-3xl lg:text-4xl font-semibold text-gray-900 dark:text-neutral-300 animate-fade-in-up">
          Ready to add and view your hostel? We've got you covered, <span class="text-blue-700 dark:text-blue-300">{{
            user.name }}</span>!
        </h2>
      ...
      <div v-if="authStore.loading"
        class="text-center text-blue-600 dark:text-blue-400 my-8 py-4 px-6 bg-blue-50 dark:bg-neutral-900 rounded-lg shadow-md border border-blue-200 dark:border-blue-700">
      ...
      <div v-else-if="authStore.error"
        class="text-center bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400 my-8 py-4 px-6 rounded-lg shadow-md border border-red-200 dark:border-red-700">
        Error: {{ authStore.error }}. Please try logging in again.
      </div>
      <div v-else-if="!user || !user.uid"
        class="text-center bg-yellow-100 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400 my-8 py-4 px-6 rounded-lg shadow-md border border-yellow-200 dark:border-yellow-700">
        You must be logged in to access this page. Please <router-link to="/login"
          class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200 font-semibold underline">log
          in</router-link>.
      </div>

      <div v-if="user && user.uid && !authStore.loading && !authStore.error"
        class="bg-blue-50 dark:bg-neutral-900 p-6 sm:p-8 rounded-xl shadow-lg border border-blue-100 dark:border-neutral-800">
        <h2
          class="text-4xl sm:text-5xl font-extrabold text-gray-800 dark:text-neutral-200 text-center mb-8 animate-fade-in-up">
          Caretaker Dashboard
        </h2>

        <div class="mb-8 flex justify-center gap-4 border-b border-gray-200 dark:border-neutral-700 pb-4">
          <button @click="currentView = 'hostelList'" :class="{
          ...
          <button @click="currentView = 'addHostelForm'" :class="{
        ...
        <div v-if="caretakerStore.loading || universityStore.loading"
          class="text-center text-blue-600 dark:text-blue-400 my-8 py-4 px-6 bg-blue-50 dark:bg-neutral-800 rounded-lg border border-blue-200 dark:border-blue-700">
          <div class="flex items-center justify-center space-x-2">
            <svg class="animate-spin h-5 w-5 text-blue-500" xmlns="http://www.w3.org/2000/svg" fill="none"
              viewBox="0 0 24 24">
              <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
        ...
        <div v-if="caretakerStore.error || universityStore.error"
          class="text-center bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400 my-8 py-4 px-6 rounded-lg shadow-md border border-red-200 dark:border-red-700">
        ...
        <div v-if="currentView === 'hostelList'">
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            <div v-if="!caretakerStore.loading && caretakerStore.hostels.length === 0"
              class="lg:col-span-2 text-center text-gray-500 dark:text-neutral-500 p-8 border rounded-xl bg-white dark:bg-neutral-900 shadow-inner">
            ...
            <div v-for="hostel in caretakerStore.hostels" :key="hostel.id" class="animate-fade-in-right">
              <div
                ...
                <div v-if="hostel.imageUrls && hostel.imageUrls.length > 0"
                  class="mt-4 flex gap-3 overflow-x-auto pb-3 border-t border-b border-gray-200 dark:border-neutral-700 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 dark:scrollbar-thumb-neutral-700 dark:scrollbar-track-neutral-800">
                  <img v-for="(imageUrl, index) in hostel.imageUrls" :key="index" :src="imageUrl" alt="Hostel Image"
                ...
                <div v-else class="text-gray-400 text-sm italic mt-4 mb-5 p-2 bg-gray-50 dark:bg-neutral-800 rounded">No
                  images available</div>
                ...
                <div
                  ...
                  <ul class="space-y-2 text-md sm:text-lg">
                    ...
                    <li class="flex gap-x-3 items-center">
                      <CheckIcon class="flex-shrink-0 w-5 h-5 text-green-500" />
                      <span class="text-gray-800 dark:text-neutral-200">
                        Location: <span class="font-medium">{{ getPlaceName(hostel.locationId,
                          universityStore.allLocations) }}</span>
                      </span>
                    </li>
                  </ul>
                  <ul class="space-y-2 text-md sm:text-lg">
                    <li class="flex gap-x-3 items-center">
                      <CheckIcon class="flex-shrink-0 w-5 h-5 text-green-500" />
                      <span class="text-gray-800 dark:text-neutral-200">
                        University: <span class="font-medium">{{ getPlaceName(hostel.universityId,
                          universityStore.universities) }}</span>
                      </span>
                    </li>
                    <li class="flex gap-x-3 items-center">
                      <CheckIcon class="flex-shrink-0 w-5 h-5 text-green-500" />
                ...
                <div
                  class="mt-6 pt-4 border-t border-gray-200 dark:border-neutral-700 flex flex-col sm:flex-row justify-end gap-3">
                  <button @click="editHostel(hostel)"
                    ...
                    <svg class="h-4 w-4" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                      <path
                        d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zm-3.646 3.646L6.5 13.086V16h2.914l6.364-6.364-2.828-2.828z" />
                    </svg>
                    Edit
                  </button>
                  <button @click="deleteHostel(hostel)"
                  ...
                  <button @click="toggleAvailability(hostel)" :class="{
        ...
        <div v-if="currentView === 'addHostelForm'">
          <form @submit.prevent="submitHostelForm"
            class="bg-white rounded-xl shadow-lg dark:bg-neutral-900 p-7 sm:p-10 border border-gray-200 dark:border-neutral-800 animate-fade-in-down">
            <h2 class="text-3xl font-bold text-gray-800 dark:text-neutral-200 mb-8 text-center">Add New Apartment Hostel
            </h2>

            <div class="grid sm:grid-cols-12 gap-6">
              <div class="sm:col-span-12 lg:col-span-3">
                <label for="university-select"
                  class="inline-block text-sm font-medium text-gray-800 dark:text-neutral-200 mb-2">
                  University <span class="text-red-500">*</span>
                </label>
              </div>
              <div class="sm:col-span-12 lg:col-span-9">
                <select id="university-select" v-model="form.selectedUniversity" @change="fetchLocationsForUniversity"
              ...
              <div class="sm:col-span-12 lg:col-span-9">
                <select id="location-select" v-model="form.selectedLocation" :disabled="locations.length === 0"
              ...
              <div class="sm:col-span-12 lg:col-span-3">
                <label for="hostel-name"
                  class="inline-block text-sm font-medium text-gray-800 dark:text-neutral-200 mb-2">
                  Hostel Name <span class="text-red-500">*</span>
                </label>
              </div>
              <div class="sm:col-span-12 lg:col-span-9">
                <input id="hostel-name" type="text" v-model="form.hostelName"
                  class="py-2.5 px-4 block w-full border border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-400 focus:ring-offset-white dark:focus:ring-offset-neutral-900"
                  required />
              </div>

              <div class="sm:col-span-12 lg:col-span-3">
                <label for="caretaker-phone-number"
                  class="inline-block text-sm font-medium text-gray-800 dark:text-neutral-200 mb-2">
                  Hostel Contact Number <span class="text-red-500">*</span>
                </label>
              </div>
              <div class="sm:col-span-12 lg:col-span-9">
                <input id="caretaker-phone-number" type="text" v-model="form.caretakerPhoneNumber"
                  class="py-2.5 px-4 block w-full border border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-400 focus:ring-offset-white dark:focus:ring-offset-neutral-900"
              ...
              <div class="sm:col-span-12 lg:col-span-9">
                <input id="rent" type="number" v-model.number="form.rent"
                  class="py-2.5 px-4 block w-full border border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-400 focus:ring-offset-white dark:focus:ring-offset-neutral-900"
                  required />
              </div>
              ...
              <div class="sm:col-span-12 lg:col-span-9">
                <input id="deposit" type="number" v-model.number="form.deposit"
                  class="py-2.5 px-4 block w-full border border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-400 focus:ring-offset-white dark:focus:ring-offset-neutral-900"
                  required />
              </div>

              <div class="sm:col-span-12 lg:col-span-3">
                <label for="hostel-type"
                  class="inline-block text-sm font-medium text-gray-800 dark:text-neutral-200 mb-2">
...
<div class="sm:col-span-12 lg:col-span-9">
  <input id="available-rooms" type="number" v-model.number="form.availableRooms" min="0"
    class="py-2.5 px-4 block w-full border border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-400 focus:ring-offset-white dark:focus:ring-offset-neutral-900"
    required />
</div>
              ...
              <div class="sm:col-span-12 lg:col-span-9">
                <select id="hostel-type" v-model="form.hostelType"
                  class="py-2.5 px-4 block w-full border border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-400 focus:ring-offset-white dark:focus:ring-offset-neutral-900"
                  required>
              ...
              <div class="sm:col-span-full">
                ...
                <div @dragover.prevent="isDragging = true" @dragleave="isDragging = false"
                  ...
                  <div v-if="form.imageFiles && form.imageFiles.length > 0"
                    class="mt-4 flex flex-wrap gap-2 justify-center">
                    <div v-for="(file, index) in form.imageFiles" :key="index"
                      class="flex items-center gap-2 p-2 border border-gray-200 dark:border-neutral-700 rounded-md bg-gray-50 dark:bg-neutral-800 shadow-sm">
            ...
            <div class="mt-8 flex justify-end gap-x-3">
              ...
              <button type="submit" :disabled="caretakerStore.loading || universityStore.loading"
            ...
            <p v-if="caretakerStore.error"
              class="text-red-600 dark:text-red-400 text-center mt-6 p-3 bg-red-100 dark:bg-red-900/20 rounded-lg">{{
              caretakerStore.error }}</p>
            <p v-if="caretakerStore.addHostelSuccess"
              class="text-green-600 dark:text-green-400 text-center mt-6 p-3 bg-green-100 dark:bg-green-900/20 rounded-lg">
              Hostel added successfully!</p>
          </form>
        </div>
      </div>
    </div>
      ...
      <div class="bg-white dark:bg-neutral-900 p-6 w-full max-w-2xl rounded-lg shadow-lg relative">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input v-model="editingHostel.name" type="text"
              class="w-full p-2 border rounded-md dark:bg-neutral-800 dark:border-neutral-600 dark:text-neutral-100" />
          </div>
          <div>
            <label class="block text-sm text-gray-700 dark:text-neutral-300 mb-1">Phone</label>
            <input v-model="editingHostel.caretakerPhoneNumber" type="text"
              class="w-full p-2 border rounded-md dark:bg-neutral-800 dark:border-neutral-600 dark:text-neutral-100" />
          </div>
          <div>
            <label class="block text-sm text-gray-700 dark:text-neutral-300 mb-1">Rent</label>
            <input v-model.number="editingHostel.price" type="number"
              class="w-full p-2 border rounded-md dark:bg-neutral-800 dark:border-neutral-600 dark:text-neutral-100" />
          </div>
          <div>
            <label class="block text-sm text-gray-700 dark:text-neutral-300 mb-1">Deposit</label>
            <input v-model.number="editingHostel.deposit" type="number"
              class="w-full p-2 border rounded-md dark:bg-neutral-800 dark:border-neutral-600 dark:text-neutral-100" />
          </div>
          <div>
            <label class="block text-sm text-gray-700 dark:text-neutral-300 mb-1">Hostel Type</label>
            <select v-model="editingHostel.hostelType"
              class="w-full p-2 border rounded-md dark:bg-neutral-800 dark:border-neutral-600 dark:text-neutral-100">
              <option value="Bedsitter">Bedsitter</option>
              <option value="One bedroom">One bedroom</option>
              <option value="Two bedroom">Two bedroom</option>
          ...
          <div>
            ...
            <input v-model.number="editingHostel.availableRooms" type="number"
              class="w-full p-2 border rounded-md dark:bg-neutral-800 dark:border-neutral-600 dark:text-neutral-100" />
          </div>
        </div>
        ...
        <div class="mt-6 flex justify-end gap-3">
          <button @click="cancelEdit"
            class="px-4 py-2 rounded bg-gray-200 dark:bg-neutral-700 text-gray-800 dark:text-white hover:bg-gray-300">Cancel</button>
          <button @click="submitEditHostelForm" class="px-4 py-2 rounded bg-blue-600 text-white hover:bg-blue-700">Save
            Changes</button>
        </div>
      </div>
    </div>
